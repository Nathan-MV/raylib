...
extern void LoadFontDefault(void)
{
    ...
    // NOTE: This memory must be freed at end! --> Done by RayCloseWindow()
    defaultFont.glyphs = (GlyphInfo *)RL_CALLOC(defaultFont.glyphCount, sizeof(GlyphInfo));
    defaultFont.recs = (RayRectangle *)RL_CALLOC(defaultFont.glyphCount, sizeof(RayRectangle));
...
#if defined(SUPPORT_FILEFORMAT_TTF) || defined(SUPPORT_FILEFORMAT_BDF)
Image GenImageFontAtlas(const GlyphInfo *glyphs, RayRectangle **glyphRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    Image atlas = { 0 };
...
#define SUPPORT_COMPRESSED_FONT_ATLAS
    ...
    byteCount += sprintf(txtData + byteCount, "static RayRectangle fontRecs_%s[%i] = {\n", fileNamePascal, font.glyphCount);
    for (int i = 0; i < font.glyphCount; i++)
    {
...
//#define SUPPORT_FONT_DATA_COPY
    ...
    byteCount += sprintf(txtData + byteCount, "    font.recs = (RayRectangle *)malloc(font.glyphCount*sizeof(RayRectangle));\n");
...
void DrawFPS(int posX, int posY)
{
    ...
    RayDrawText(TextFormat("%2i FPS", fps), posX, posY, 20, color);
}
...
static Font LoadBMFont(const char *fileName)
{
    ...
    for (int i = 0; i < pageCount; i++)
    {
        imFonts[i] = RayLoadImage(TextFormat("%s/%s", GetDirectoryPath(fileName), imFileName[i]));
    ...
    if (pageCount > 1)
    {
        ...
        {
            RayRectangle srcRec = { 0.0f, 0.0f, (float)imWidth, (float)imHeight };
            RayRectangle destRec = { 0.0f, (float)imHeight*(float)i, (float)imWidth, (float)imHeight };
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        {
            // Get character rectangle in the font atlas texture
            font.recs[i] = (RayRectangle){ (float)charX, (float)charY + (float)imHeight*pageID, (float)charWidth, (float)charHeight };
