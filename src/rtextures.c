...
void ImageDraw(Image *dst, Image src, RayRectangle srcRec, RayRectangle dstRec, Color tint)
{
    ...
    else
    {
        ...
        if ((dst->mipmaps > 1) && (src.mipmaps > 1))
        {
            ...
            RayRectangle mipmapSrcRec = srcRec;
            mipmapSrcRec.width /= 2;
            mipmapSrcRec.height /= 2;
            mipmapSrcRec.x /= 2;
            mipmapSrcRec.y /= 2;

            RayRectangle mipmapDstRec = dstRec;
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, mipmapped, faceRecs[i], (RayRectangle){ 0, (float)size*i, (float)size, (float)size }, WHITE);
