...
static RayRectangle texShapesRec = { 0.0f, 0.0f, 1.0f, 1.0f };    // Texture source rectangle used on shapes drawing
...
void SetShapesTexture(Texture2D texture, RayRectangle source)
{
    ...
    if ((texture.id == 0) || (source.width == 0) || (source.height == 0))
    {
        ...
        texShapesRec = (RayRectangle){ 0.0f, 0.0f, 1.0f, 1.0f };
    }
    else
    {
        texShapes = texture;
...
RayRectangle GetShapesTextureRectangle(void)
{
    return texShapesRec;
}
...
{
    DrawRectangleGradientEx((RayRectangle){ (float)posX, (float)posY, (float)width, (float)height }, top, bottom, bottom, top);
}

// Draw a horizontal-gradient-filled rectangle
void DrawRectangleGradientH(int posX, int posY, int width, int height, Color left, Color right)
{
    DrawRectangleGradientEx((RayRectangle){ (float)posX, (float)posY, (float)width, (float)height }, left, left, right, right);
}

// Draw a gradient-filled rectangle
void DrawRectangleGradientEx(RayRectangle rec, Color topLeft, Color bottomLeft, Color topRight, Color bottomRight)
{
    rlSetTexture(GetShapesTexture().id);
    RayRectangle shapeRect = GetShapesTextureRectangle();
...
void DrawRectangleRoundedLines(RayRectangle rec, float roundness, int segments, Color color)
{
    DrawRectangleRoundedLinesEx(rec, roundness, segments, 1.0f, color);
}

// Draw rectangle with rounded edges outline
void DrawRectangleRoundedLinesEx(RayRectangle rec, float roundness, int segments, float lineThick, Color color)
{
